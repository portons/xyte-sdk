...
if [[ ! -f "$REPO_ROOT/dist/bin/xyte.js" ]]; then
  echo "Build output missing at $REPO_ROOT/dist/bin/xyte.js" >&2
  echo "Run: npm run build" >&2
  exit 1
fi

SCREENS=(setup config dashboard spaces devices incidents tickets copilot)
META_KEYS=(inputState queueDepth droppedEvents transitionState refreshState navigationMode activePane availablePanes tabId tabOrder tabNavBoundary renderSafety tableFormat)
TMP_CFG="$(mktemp -d)"
...
for screen in "${SCREENS[@]}"; do
  ...
  output="$(XYTE_SDK_CONFIG_DIR="$TMP_CFG" XYTE_SDK_KEYCHAIN_BACKEND=memory "${cmd[@]}")"
