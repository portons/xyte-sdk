...
interface LegacyTenantProfileV1 {
  id: string;
  name?: string;
  hubBaseUrl?: string;
  entryBaseUrl?: string;
  openaiCompatibleBaseUrl?: string;
  defaultLLMProvider?: 'openai' | 'anthropic' | 'openai-compatible';
...
interface LegacyProfileStoreDataV1 {
  version?: 1;
  activeTenantId?: string;
  globalDefaultLLMProvider?: 'openai' | 'anthropic' | 'openai-compatible';
  globalDefaultLLMModel?: string;
  tenants?: LegacyTenantProfileV1[];
}
...
function normalizeTenant(raw: LegacyTenantProfileV1): TenantProfile {
...
export class FileProfileStore implements ProfileStore {
  ...
  async getData(): Promise<ProfileStoreData> {
    try {
      ...
      const parsed = JSON.parse(content) as LegacyProfileStoreDataV1 | ProfileStoreData;
  ...
  private normalize(input: LegacyProfileStoreDataV1 | ProfileStoreData): ProfileStoreData {
    const tenants = (Array.isArray(input.tenants) ? input.tenants : [])
      .filter((tenant): tenant is LegacyTenantProfileV1 => Boolean(tenant?.id))
