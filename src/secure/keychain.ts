...
function legacyAccountName(tenantId: string, provider: SecretProvider): string {
...
class DarwinSecurityKeychain implements KeychainStore {
  ...
  async getSlotSecret(tenantId: string, provider: SecretProvider, slotId: string): Promise<string | undefined> {
    const account = accountName(tenantId, provider, slotId);
    const primary = await this.readAccount(account);
    if (primary) {
      return primary;
    }

    if (slotId === DEFAULT_SLOT_ID) {
      return this.readAccount(legacyAccountName(tenantId, provider));
    }
    return undefined;
  }
  ...
  async clearSlotSecret(tenantId: string, provider: SecretProvider, slotId: string): Promise<void> {
    ...
    if (slotId === DEFAULT_SLOT_ID) {
      await this.deleteAccount(legacyAccountName(tenantId, provider));
    }
  }
...
class LinuxSecretToolKeychain implements KeychainStore {
  ...
  async getSlotSecret(tenantId: string, provider: SecretProvider, slotId: string): Promise<string | undefined> {
    const account = accountName(tenantId, provider, slotId);
    const primary = await this.lookupAccount(account);
    if (primary) {
      return primary;
    }

    if (slotId === DEFAULT_SLOT_ID) {
      return this.lookupAccount(legacyAccountName(tenantId, provider));
    }
    return undefined;
  }
  ...
  async clearSlotSecret(tenantId: string, provider: SecretProvider, slotId: string): Promise<void> {
    ...
    if (slotId === DEFAULT_SLOT_ID) {
      await this.clearAccount(legacyAccountName(tenantId, provider));
    }
  }
...
export class MemoryKeychain implements KeychainStore {
  ...
  async getSlotSecret(tenantId: string, provider: SecretProvider, slotId: string): Promise<string | undefined> {
    const primary = this.values.get(accountName(tenantId, provider, slotId));
    if (primary !== undefined) {
      return primary;
    }

    if (slotId === DEFAULT_SLOT_ID) {
      return this.values.get(legacyAccountName(tenantId, provider));
    }
    return undefined;
  }
  ...
  async clearSlotSecret(tenantId: string, provider: SecretProvider, slotId: string): Promise<void> {
    ...
    if (slotId === DEFAULT_SLOT_ID) {
      this.values.delete(legacyAccountName(tenantId, provider));
    }
  }
...
export async function createKeychainStore(): Promise<KeychainStore> {
  if (process.env.XYTE_SDK_KEYCHAIN_BACKEND === 'memory') {
  ...
  throw new Error(
    'No supported OS keychain backend found. Use macOS keychain, libsecret (secret-tool), or set XYTE_SDK_KEYCHAIN_BACKEND=memory for tests.'
  );
}
